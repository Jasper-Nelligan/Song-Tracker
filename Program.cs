/*
 * A simple C# .NET command line program that tracks your favourite songs throughout time.
 * I used https://json2csharp.com to convert Json data into class structure
 * (C) Jasper Nelligan 2021
 */
using System;
...
namespace Song_Tracker
{
    public class Program
    {
        ...
        private static Entry CreateNewEntry(Entry prevEntry)
        {
            Entry newEntry = null;
            while (true)
            {
        ...
        /// </returns>
        private static Entry AddNewSong(Entry prevEntry)
        {
            int pos = 0;
            if (prevEntry == null)
            {
                prevEntry = new Entry(string.Empty, new List<Song>());
            }
        ...
        /// A new instance of Entry, null if user decides not to change anything.
        /// </returns>
        private static Entry DelSong(Entry prevEntry)
        {
        ...
        private static void SearchFavourites(in List<Entry> entries)
        {
